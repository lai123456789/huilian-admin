...
import { getCustomsMasterOperRecord } from '@/api/customs/customs-master-oper-record'
...
export default {
  ...
  created() {
      this.annexConfig = {
        ...
        permissionsAnnex: {
          add: constant.PERMISSION_CUS_DECLAR_ANNEX_UPLOAD,
          del: constant.PERMISSION_CUS_DECLAR_ANNEX_DEL
        },
        fileModule: constant.FILE_MODULE_CUS_DECLAR
      }
    this.operRecordConfig = {
      pagePort: getCustomsMasterOperRecord
    }
  },
  methods: {
    createPage() {
...
<template>
  ...
  >
    <business-details-drawer-tabs
      v-model="tabsActive"
      :options="tabOptions"
      :attrs="{ tableRow, deleteFn: handleDel, reLoad, orderType, annexConfig, operRecordConfig}"
    />
  </business-details-drawer>
</template>
