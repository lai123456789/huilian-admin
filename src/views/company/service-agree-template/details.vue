  ...
  methods: {
    ...
    async getDetailsData(cb) {
      ...
      const [dicts, info] = await Promise.all([
        ...
        getCompanyCusContractTemplateById(this.tableRow.id)
      ])
      const { status, companyCusContractTemplateDetailVO, createByName, createTime } = info.data
      const { agencyPriceConfig } = companyCusContractTemplateDetailVO
      this.dicts = dicts
      ...
      this.showBtns = {
        edit: this.permissions[constant.PERMISSION_SERVICE_AGREE_TEMPLATE_EDIT],
        del: this.permissions[constant.PERMISSION_SERVICE_AGREE_TEMPLATE_DEL]
      }
      // 先赋值
      this.request = () => {
    ...
    handleDataAssemble(formData) {
      // companyCusContractTemplateDetailVO: 包含了服务费、货款、税款的数据
      // companyCusSettlementDayTemplateVOList: 包含了服务费、货款、税款结算方式的数据组
...
<template>
  <div
    v-loading="loading"
    class="common-operate cus-contract-template-details"
  ...
  >
              ...
              <base-row :gutter="10">
                <base-col :span="6">
                  <base-form-item label="模板名称" prop="templateName">
                    <base-input v-model="form.templateName" disabled placeholder="请输入模板名称" />
                  </base-form-item>
                </base-col>

                <base-col :span="6">
                  <base-form-item label="状态" prop="status">
                    <base-select v-model="form.status" disabled placeholder="请选择状态" :options="dicts[constant.DICTS_ENABLE_DISABLE]" />
                  </base-form-item>
                </base-col>
    ...
    <div class="common-operate-b">
      <div class="common-operate-b-wrap">
        <el-button v-if="showBtns.del" icon="el-icon-delete" type="danger" @click="handleDelete">删除</el-button>
        <el-button v-if="showBtns.edit" icon="el-icon-edit" type="warning" @click="handleEdit">编辑</el-button>
      </div>
    </div>
