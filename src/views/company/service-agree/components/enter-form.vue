<script>
import { getCompanyCusContractById, saveCompanyCusContract, updateCompanyCusContract } from '@/api/company/company-cus-contract'
import { getCompanyCusContractTemplateById } from '@/api/company/company-cus-contract-template'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        customerName: [{ required: true, message: '签约客户名称必填' }],
        principalName: [{ required: true, message: '签约主体名称必填' }],
        signDate: [{ required: true, message: '签约日期必填' }],
  ...
  methods: {
    getFormData() {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          type && getCompanyCusContractById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
          ...
          if (type) {
            ...
            // 复制功能
            if (this.isCopy) {
              const { id } = this.extraObj
              getCompanyCusContractById(id).then(info => {
                Object.assign(this.form, this.handleDataAssemble(info.data || {}, 'get'))
                this.form.contractNo = ''
              })
            } else if (this.isJump) {
              const { id, companyName } = this.extraObj
    ...
    handleDataAssemble(formData, type = 'submit') {
      ...
      if (type === 'submit') {
        ...
        const companyCusContractSaveDTO = {
          ...
          remark,
          // 汇率约定
          companyCusContractDetailRateDTO: {
            exchangeRateType,
            exchangeRateTime,
            cargoRateType,
            taxRateType
          ...
          // 滞纳金约定
          companyCusContractDetailOverdueDTO: {
            // 把百分数转成小数
            overdueRate: new BigNumber(overdueRate).div(100).toNumber(),
            interestFreeDay
          },
          // 货款约定
          companyCusContractDetailCargoDTO: {
            cargoAdvanceFlag,
            cargoInterestDay,
            cargoAdvanceRateType,
            // 把百分数转成小数
            ...
            cargoSettlementMode,
            // 货款：结算方式
            companyCusSettlementDayDTOList: this.cargoSettleObj.settleState.map(v => {
          ...
          // 服务费约定
          companyCusContractDetailAgencyDTO: {
            agencyCollectFlag,
            agencyMark,
            agencyPriceConfig,
            // 把百分数转成小数，保留2位
          ...
          // 税费约定
          companyCusContractDetailTaxDTO: {
            taxAdvanceFlag,
            taxActualFlag,
            taxHappenDate,
            taxSettlementMode,
          ...
          // 附件条件
          companyCusContractDetailOtherDTO: {
            deliveryFlag,
            invoiceFlag,
            refundTaxFlag,
            agencyFeeFlag,
    ...
    handleTemplateNameSelect(rows) {
      const { id, templateNo } = rows[0]
      this.form.templateNo = templateNo || ''
      getCompanyCusContractTemplateById(id).then(res => {
        const info = res.data
        info.companyCusContractDetailVO = info.companyCusContractTemplateDetailVO
        info.companyCusSettlementDayVOList = info.companyCusSettlementDayTemplateVOList
        Object.assign(this.form, this.handleDataAssemble(info, 'get'))
      })
    },
    ...
    // handleoCollectFlagChange(value, type) {
    //   // 服务费约定是否收取
    //   if (type === 'agencyCollectFlag') {
    //     Object.assign(this.form, {
    //       agencyRate: 0,
    //       extraAgencyMinFee: '',
    //       agencyMinFee: '',
...
<template>
  <base-dialog
    ...
    width="98%"
    custom-class="common-form-dialog cus-contract-dialog"
    :title="
        ...
        <base-row :gutter="10">
          ...
          <base-col :span="6">
            <base-form-item label="协议编号" prop="contractNo">
              <base-input v-model="form.contractNo" placeholder="请输入协议编号" />
            </base-form-item>
          </base-col>
          ...
          <base-col :span="6">
            <base-form-item label="合同名称" prop="contractName">
              <base-input v-model="form.contractName" placeholder="请输入合同名称" />
            </base-form-item>
          </base-col>
          ...
          <base-col :span="6">
            <base-form-item label="签约日期" prop="signDate">
              <base-date-picker v-model="form.signDate" placeholder="请输入签约日期" />
            </base-form-item>
          </base-col>

          <base-col :span="6">
            <base-form-item label="生效日期" prop="effectiveDate">
              <base-date-picker v-model="form.effectiveDate" placeholder="请输入生效日期" />
            </base-form-item>
          </base-col>

          <base-col :span="6">
            <base-form-item label="到期日期" prop="expireDate">
              <base-date-picker v-model="form.expireDate" placeholder="请输入到期日期" />
            </base-form-item>
          </base-col>
