<script>
import { getCompanySupDeliveryById, saveCompanySupDelivery, updateCompanySupDelivery } from '@/api/company/company-sup-delivery'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        deliveryAddress: [{ required: true, message: '交货地址', trigger: 'blur' }]
      },
      dicts: {}
    }
  },
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getCompanySupDeliveryById(this.mainRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
  ...
  methods: {
    handleSave(done) {
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          const formData = Object.assign(this.form, { supplierId: this.tableRow.id })
          if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
            saveCompanySupDelivery(formData)
              .then(() => {
            ...
            updateCompanySupDelivery(formData)
              .then(() => {
...
<template>
  ...
  >
    <base-form ref="formRef" :model="form" :rules="rules" label-width="90px" size="mini">
      <base-row :gutter="10">
        <base-col :span="24">
          <base-form-item label="交货公司" prop="deliveryCompany">
            <base-input v-model="form.deliveryCompany" placeholder="请输入交货公司"></base-input>
          </base-form-item>
        </base-col>

        <base-col :span="24">
          <base-form-item label="提货地址" prop="deliveryAddress">
            <base-input v-model="form.deliveryAddress" placeholder="请输入提货地址"> </base-input>
          </base-form-item>
        </base-col>
