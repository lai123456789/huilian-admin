<script>
import {
  getCompanyPrincipalById,
  saveCompanyPrincipal,
  updateCompanyPrincipal,
  isExistsCompanyPrincipalByCompanyId
} from '@/api/company/company-principal'
...
export default {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getCompanyPrincipalById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
          if (this.type === enums.FORM_TYPE.getFieldByKey('MODIFY')) {
            const { id, companyId, companyNo, easNo, remark, companyInfoVO } = info.data
            const {
              id: companyInfoId,
              companyName,
              companyNameEn,
  ...
  methods: {
    async onCompanyNameSelect(rows) {
      this.form = clone(form, true)
      const { id } = rows[0]
      const res = await isExistsCompanyPrincipalByCompanyId(id)
      if (res.data) {
        this.$message.warning('报关行已存在')
      }
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          ...
          if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
            ...
            updateCompanyPrincipal(formData)
              .then(() => {
        ...
        <base-row :gutter="10">
          ...
          <base-col :span="6">
            <base-form-item label="企业简称" prop="companyShortName">
              <base-input v-model="form.companyShortName" placeholder="请输入企业简称" />
            </base-form-item>
          </base-col>
