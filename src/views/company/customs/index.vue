<!-- 报关行 -->
<script>
import { getCompanyCustoms, deleteCompanyCustomsById } from '@/api/company/company-customs'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getCompanyCustoms({ ...pager, ...transforArrayPayload(params) })
        return {
          data: data.records,
          total: data.total
        }
  ...
  created() {
    this.helpConfig = {
      show: this.permissions[constant.PERMISSION_CUSTOMS],
      mark: constant.PERMISSION_CUSTOMS
    }
    this.buttonsConfig = {
      add: {
        visible: this.permissions[constant.PERMISSION_CUSTOMS_ADD],
        handler: this.handleAdded
      },
      edit: {
        visible: this.permissions[constant.PERMISSION_CUSTOMS_EDIT],
        handler: this.handleEdit
      },
      del: {
        visible: this.permissions[constant.PERMISSION_CUSTOMS_DEL],
  ...
  methods: {
    ...
    handleDel(row, cb) {
      this.$confirm('是否确认删除所选记录?')
        .then(() => {
          deleteCompanyCustomsById(row.id).then(() => {
            // 清空表格全部选中的数据
            this.$refs['companyCustomsRef'].handleTableClearCheckbox(true)
    ...
    reLoad(isClearSelect) {
      this.$refs['companyCustomsRef'].refresh(isClearSelect)
    }
  }
}
</script>
...
<template>
  <div class="company-customs">
    <!-- 列表页 -->
    <base-page
      ref="companyCustomsRef"
