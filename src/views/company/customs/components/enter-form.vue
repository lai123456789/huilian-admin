<script>
import {
  getCompanyCustomsById,
  saveCompanyCustoms,
  updateCompanyCustoms,
  isExistsCompanyCustomsByCompanyId
} from '@/api/company/company-customs'
...
export default {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getCompanyCustomsById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
  ...
  methods: {
    async onCompanyNameSelect(rows) {
      this.form = clone(form, true)
      const { id } = rows[0]
      const res = await isExistsCompanyCustomsByCompanyId(id)
      if (res.data) {
