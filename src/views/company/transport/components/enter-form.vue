<script>
import {
  getCompanyTransportById,
  saveCompanyTransport,
  updateCompanyTransport,
  isExistsCompanyTransportByCompanyId
} from '@/api/company/company-transport'
...
export default {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getCompanyTransportById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
  ...
  methods: {
    async onCompanyNameSelect(rows) {
      this.form = clone(form, true)
      const { id } = rows[0]
      const res = await isExistsCompanyTransportByCompanyId(id)
      if (res.data) {
        this.$message.warning('承运商已存在')
      }
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          ...
          if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
            ...
            updateCompanyTransport(formData)
              .then(() => {
    ...
    handleSubmitForm(form) {
      ...
      return companyTransportSaveDTO
    },
    getFormInstance() {
