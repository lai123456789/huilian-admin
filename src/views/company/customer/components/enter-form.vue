...
import {
  ...
  isExistsCompanyCustomerByCompanyId
} from '@/api/company/company-customer'
...
export default {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getCompanyCustomerById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
  ...
  methods: {
    async onCompanyNameSelect(rows) {
      this.form = clone(form, true)
      const { id } = rows[0]
      const res = await isExistsCompanyCustomerByCompanyId(id)
      if (res.data) {
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          ...
          if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
            ...
            updateCompanyCustomer(formData)
              .then(() => {
