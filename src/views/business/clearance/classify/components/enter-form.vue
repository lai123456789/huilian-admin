<script>
import { getCustomsDeclarById, updateCustomsDeclar, saveCustomsDeclar, batchDeclareCustomsDeclar } from '@/api/customs/customs-declar'
...
export default {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        if (this.isBatchClassify) {
          const data = []
          this.batchClassifyRows.forEach(({ id, customsCode, productBrand, productModel }) => {
            // 取有海关编码的第一条数据
            if (customsCode && !rowId) {
              rowId = id
            }
            data.push({ productBrand, productModel })
          })
          this.$nextTick(() => {
        ...
        const { DICTS_IS_3C, DICTS_CUSTOMS_ELEMENT_BRAND_TYPE, DICTS_CUSTOMS_ELEMENT_EXPORT_BENEFIT } = constant
        Promise.all([
          getDataDictionary([DICTS_IS_3C, DICTS_CUSTOMS_ELEMENT_BRAND_TYPE, DICTS_CUSTOMS_ELEMENT_EXPORT_BENEFIT]),
          publicValueSysPublicParamByPublicKey(constant.PARAMS_ORDER_CLASSIFIED),
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getCustomsDeclarById(rowId)
        ]).then(([dicts, defaultConfig, info]) => {
          this.dicts = dicts
          this.defaultConfig = defaultConfig.data || {}
          if (this.type === enums.FORM_TYPE.getFieldByKey('MODIFY')) {
            const { customsCode, productModel, productBrand, cccFlag, cccNo, ciqCode, ciqName, customsDeclarElementVOList } = info.data
            Object.assign(
              this.form,
              {
                classifyElements: customsDeclarElementVOList
              },
              info.data
            )
  ...
  methods: {
    handleProductModelBlur() {
      this.assignProductModelName()
      this.assignElementNameValueKey()
    },
    handleProductModelClear() {
    ...
    handleProductBrandBlur() {
      this.assignElementNameValueKey()
    },
    handleProductBrandClear() {
      this.assignElementNameValueKey()
    },
      ...
      if (productBrand) {
        ...
        if (productBrandNothing.includes(productBrand)) {
          ...
          if (!elementValue) {
            ...
            // 如果为空时，productBrandNameZhEn = '无中文品牌'和 productBrandType = ''
            if (res.data) {
    ...
    async assignElementNameValueKey() {
      await this.assignProductBrand()
      if (this.form.classifyElements.length > 0) {
        this.form.classifyElements.forEach(element => {
          if (ENUM[element.elementName] && ENUM[element.elementName].valueKey) {
            element.elementValue =
              typeof ENUM[element.elementName].valueKey === 'function'
                ? ENUM[element.elementName].valueKey(this)
                : this[ENUM[element.elementName].valueKey]
          }
        })
        this.handleClassifyElementSplice()
      }
          ...
          this.getBlurbFormInstance().validate(async (blurbFormValid, blurbFormMsgs) => {
            if (blurbFormValid) {
              ...
              if (this.type === enums.FORM_TYPE.getFieldByKey('MODIFY')) {
                ...
                const { productBrand, productModel, cccNo, cccFlag, ciqCode, ciqName } = this.blurbForm
                const formData = {
                  id,
                  customsCode,
                  customsName,
                  ...
                  ciqCode,
                  ciqName,
                  customsDeclarElementDTOList: classifyElements || [],
                  remark
                }
                try {
                  if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
                    await saveCustomsDeclar(formData)
                  } else {
                    if (this.isBatchClassify) {
                      formData.ids = this.batchClassifyRows.map(row => row.id)
                      await batchDeclareCustomsDeclar(formData)
                    } else {
                      await updateCustomsDeclar(formData)
                    }
                  }
                  if (!declareStatus) {
      ...
      <base-col :span="5" style="padding-right: 10px">
          ...
          <base-row>
            <base-col v-if="!isBatchClassify" :span="24">
              <base-form-item label="型号" prop="productModel">
                <business-autocomplete-basic
                  v-model="blurbForm.productModel"
                  size="mini"
            ...
            <base-col v-if="!isBatchClassify" :span="24">
              <base-form-item label="品牌" prop="productBrand">
                <base-input v-model="blurbForm.productBrand" @blur="handleProductBrandBlur" @clear="handleProductBrandClear" />
              </base-form-item>
            </base-col>
        ...
        <div v-if="isBatchClassify">
          <base-edit-table
            ...
            :show-delete="false"
            :show-checkbox="false"
            :name="constant.TABLE_NAME_CLASSIF_BATCH"
          />
        </div>
      </base-col>
      <base-col :span="19">
