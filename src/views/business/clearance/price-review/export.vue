<!-- 出口审价 -->
<script>
import { getOrderExportHiPrice, resetHiPriceOrderExportHiPrice } from '@/api/order/order-export-hi-pirce'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getOrderExportHiPrice({ ...pager, ...transforArrayPayload(params) })
        return {
          data: data.records,
          total: data.total
        }
  ...
  created() {
    this.helpConfig = {
      show: this.permissions[constant.PERMISSION_PRICE_REVIEW_EXPORT],
      mark: constant.PERMISSION_PRICE_REVIEW_EXPORT
    }
    this.buttonsConfig = {
      mores: [
        {
          label: '重置审价',
          visible: this.permissions[constant.PERMISSION_PRICE_REVIEW_EXPORT_SHENJIA],
          handler: row => this.handleReviewPrice(row)
        },
        {
          label: '审价通过',
          visible: this.permissions[constant.PERMISSION_PRICE_REVIEW_EXPORT_PASS],
          type: 'success',
          handler: row => this.handlePass(row)
        },
        {
          label: '审价不通过',
          visible: this.permissions[constant.PERMISSION_PRICE_REVIEW_EXPORT_ROLLBACK],
          type: 'danger',
  ...
  methods: {
    ...
    reLoad(isClearSelect) {
      this.$refs['exportHiPircerRef'].refresh(isClearSelect)
    }
  }
}
</script>
...
<template>
  <div class="export-hi-pircer">
    <!-- 列表页 -->
    <base-page
      ref="exportHiPircerRef"
      :name="constant.TABLE_NAME_PRICE_REVIEW_EXPORT"
      :columns="columns"
      :request="request"
