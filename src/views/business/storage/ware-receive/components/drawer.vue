...
import { getWareReceivingOperRecord } from '@/api/warehouse/ware-receiving-oper-record'
...
export default {
  ...
  created() {
    this.annexConfig = {
      ...
      permissionsAnnex: {
        add: constant.PERMISSION_WARE_RECEIVE_ANNEX_UPLOAD,
        del: constant.PERMISSION_WARE_RECEIVE_ANNEX_DEL
      },
      fileModule: constant.FILE_MODULE_WARE_RECEIVE
    }
    this.operRecordConfig = {
      pagePort: getWareReceivingOperRecord
    }
  },
  methods: {
    createPage() {
...
<template>
  ...
  >
    <business-details-drawer-tabs
      v-model="tabsActive"
      :options="tabOptions"
      :attrs="{ tableRow, deleteFn: handleDel, reLoad, annexConfig, operRecordConfig }"
    />
  </business-details-drawer>
</template>
