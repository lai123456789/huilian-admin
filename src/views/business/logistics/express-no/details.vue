<script>
import {
  getWareDeliveryExpressById,
  getWareDeliveryExpressAuditList,
  auditWareDeliveryExpress
} from '@/api/warehouse/ware-delivery-express'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getWareShipping({
          ...
          ...transforArrayPayload(
            Object.assign({ deliveryId: this.tableRow.id, shippingType: enums.SHIPPING_TYPE.getFieldByKey('EXPRESS_DELIVERY') }, params)
          )
        })
        return {
          data: data.records,
      ...
      requestApi: {
        list: () => getWareDeliveryExpressAuditList(this.tableRow.id),
        save: auditWareDeliveryExpress
      },
  ...
  mounted() {
    this.showAuditDetailsBtn = this.permissions[constant.PERMISSION_EXPRESS_NO_AUDIT]
    this.getDetailsData()
  },
  methods: {
    // 获取详情数据
    async getDetailsData(cb) {
      this.$nextTick(() => {
        this.loading = true
        Promise.all([getDataDictionary([]), getWareDeliveryExpressById(this.tableRow.id)]).then(([dicts, info]) => {
          ...
          this.showBtns = {
            edit: this.permissions[constant.PERMISSION_EXPRESS_NO_EDIT],
            del: this.permissions[constant.PERMISSION_EXPRESS_NO_DEL]
          }
...
<template>
  <div
    v-loading="loading"
    class="common-operate express-no is-details-adaptive"
              ...
              <base-row :gutter="10">
                ...
                <base-col :span="6">
                  <base-form-item label="快递单号" prop="expressNo">
                    <base-input v-model="form.expressNo" disabled placeholder="请输入快递单号" />
                  </base-form-item>
                </base-col>

                <base-col :span="6">
                  <base-form-item label="发货人" prop="expressPerson">
                    <base-input v-model="form.expressPerson" disabled placeholder="请输入发货人" />
                  </base-form-item>
                </base-col>
            ...
            <div class="common-operate-block is-special">
              ...
              <div class="common-operate-block-body">
                <base-page
                  ...
                  show-footer
                  :name="constant.TABLE_NAME_EXPRESS_NO_DETAILS"
                />
              </div>
            </div>
          </base-form>
