...
import { getWareDeliverySendOperRecord } from '@/api/warehouse/ware-delivery-send-oper-record'
...
export default {
  ...
  created() {
    this.annexConfig = {
      ...
      permissionsAnnex: {
        add: constant.PERMISSION_DISPATCH_NO_ANNEX_UPLOAD,
        del: constant.PERMISSION_DISPATCH_NO_ANNEX_DEL
      },
      fileModule: constant.FILE_MODULE_DISPATCH_NO
    }
    this.operRecordConfig = {
      pagePort: getWareDeliverySendOperRecord
    }
  },
  methods: {
    createPage() {
...
<template>
  ...
  >
    <business-details-drawer-tabs
      v-model="tabsActive"
      :options="tabOptions"
      :attrs="{ tableRow, deleteFn: handleDel, reLoad, operRecordConfig, annexConfig }"
    />
  </business-details-drawer>
</template>
