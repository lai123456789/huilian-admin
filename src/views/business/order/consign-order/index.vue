<!-- 代销订单 -->
<script>
import { getOrderAgentSale, deleteOrderAgentSaleById } from '@/api/order/order-agent-sale'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getOrderAgentSale({ ...pager, ...transforArrayPayload(params) })
        return {
          data: data.records,
          total: data.total
        }
  ...
  created() {
    this.helpConfig = {
      show: this.permissions[constant.PERMISSION_BUS_CONSIGN],
      mark: constant.PERMISSION_BUS_CONSIGN
    }
    this.buttonsConfig = {
      add: {
        visible: this.permissions[constant.PERMISSION_BUS_CONSIGN_ADD],
        handler: this.handleAdded
      },
      edit: {
        visible: this.permissions[constant.PERMISSION_BUS_CONSIGN_EDIT],
        handler: this.handleEdit
      },
      del: {
        visible: this.permissions[constant.PERMISSION_BUS_CONSIGN_DEL],
        handler: this.handleDel
      },
      mores: [
        {
          label: '复制',
          visible: this.permissions[constant.PERMISSION_BUS_CONSIGN_COPY],
  ...
  methods: {
    ...
    handleDel(row, cb) {
      this.$confirm('是否确认删除所选记录?')
        .then(() => {
          deleteOrderAgentSaleById(row.id).then(() => {
            // 清空表格全部选中的数据
            this.$refs['agentSaleOrderRef'].handleTableClearCheckbox(true)
    ...
    reLoad(isClearSelect) {
      this.$refs['agentSaleOrderRef'].refresh(isClearSelect)
    }
  }
}
</script>
...
<template>
  <div class="agent-sale-order">
    <!-- 列表页 -->
    <base-page
      ref="agentSaleOrderRef"
      :name="constant.TABLE_NAME_BUS_CONSIGN"
      :columns="columns"
      :request="request"
