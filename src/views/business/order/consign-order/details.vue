<script>
import { getOrderAgentSaleById, getOrderAgentSaleAuditList, auditOrderAgentSale } from '@/api/order/order-agent-sale'
import { deleteOrderDeliveryTakeById } from '@/api/order/order-delivery-take'
import { updatePriceOrderAgentSaleDetail, getOrderAgentSaleDetail } from '@/api/order/order-agent-sale-detail'
...
export default {
  name: 'Details',
  components: { EnterForm, EnterFormModifyPrice, EnterFormTake },
  props: {
    tableRow: {
      type: Object,
      default: () => ({})
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getOrderAgentSaleDetail({
          ...pager,
      ...
      requestApi: {
        list: () => getOrderAgentSaleAuditList(this.tableRow.id),
        save: auditOrderAgentSale
      },
      showBtns: {
        edit: false,
        del: false
  ...
  mounted() {
    this.showAuditDetailsBtn = this.permissions[constant.PERMISSION_BUS_CONSIGN_AUDIT]
    this.getDetailsData()
  },
  methods: {
    // 获取详情数据
    async getDetailsData(cb) {
      this.$nextTick(() => {
        ...
        Promise.all([getDataDictionary([DICTS_TAKE_DELIVERY_TYPE, DICTS_WHETHER_STATUS]), getOrderAgentSaleById(this.tableRow.id)]).then(
          ([dicts, info]) => {
            const { receivingStatus, shippingStatus, createByName, createTime, orderDeliveryTakeVOList } = info.data
            this.dicts = dicts
            ...
            this.showBtns = {
              edit: this.permissions[constant.PERMISSION_BUS_CONSIGN_EDIT],
              del: this.permissions[constant.PERMISSION_BUS_CONSIGN_DEL]
            }
    ...
    async handleModifyPriceSave(form, done) {
      ...
      await updatePriceOrderAgentSaleDetail({ orderDetailId, price, totalMoney })
...
<template>
  <div
    v-loading="loading"
    class="common-operate agent-sale-order"
          ...
          <base-form ref="formRef" :model="form" label-width="90px" size="mini">
            ...
            <div class="common-operate-block is-special">
              ...
              <base-page
                ...
                height="380px"
                show-footer
                :name="constant.TABLE_NAME_BUS_CONSIGN_DETAILS"
              />
            </div>
