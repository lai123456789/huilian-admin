<script>
import { getOrderAgentPurchaseById, getOrderAgentPurchaseAuditList, auditOrderAgentPurchase } from '@/api/order/order-agent-purchase'
...
import { getOrderAgentPurchaseDetail, updatePriceOrderAgentPurchaseDetail } from '@/api/order/order-agent-purchase-detail'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getOrderAgentPurchaseDetail({
          ...pager,
      ...
      requestApi: {
        list: () => getOrderAgentPurchaseAuditList(this.tableRow.id),
        save: auditOrderAgentPurchase
      },
      showBtns: {
        edit: false,
        del: false,
      ...
      businessExport: {
        visible: false,
        request: async () => {
          const { data } = await getSysReportByMenuMarkSysReportConfig({ menuMark: constant.PERMISSION_BUS_ORDER_DOM_PUR_EXPORT })
          return { data }
        }
      }
    }
  ...
  mounted() {
    this.showAuditDetailsBtn = this.permissions[constant.PERMISSION_BUS_ORDER_DOM_PUR_AUDIT]
    this.getDetailsData()
  },
  methods: {
    // 获取详情数据
    async getDetailsData(cb) {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          getOrderAgentPurchaseById(this.tableRow.id),
          ...
          this.showBtns = {
            edit: this.permissions[constant.PERMISSION_BUS_ORDER_DOM_PUR_EDIT],
            del: this.permissions[constant.PERMISSION_BUS_ORDER_DOM_PUR_DEL],
            export: this.permissions[constant.PERMISSION_BUS_ORDER_DOM_PUR_EXPORT]
          }
    ...
    async handleModifyPriceSave(form, done) {
      ...
      await updatePriceOrderAgentPurchaseDetail({ orderDetailId, price, totalMoney })
    ...
    detailsReLoad(isClearSelect) {
      this.$refs['orderAgentPurchaseDetailsRef'].refresh(isClearSelect)
    },
    // 修改某一项时，刷新列表页和详情页
...
<template>
  <div
    v-loading="loading"
    class="common-operate domestic-order"
        ...
        <div class="common-operate-l-main">
          <base-form ref="formRef" :model="form" label-width="90px" size="mini">
              ...
              <base-row :gutter="10">
                  ...
                  <base-form-item label="服务协议" prop="cusContractNo">
                    <business-autocomplete-basic
                      ...
                      :params="{
                        ...
                        tradeMode: [enums.TRADE_MODE.getFieldByKey('PURCHASE_AGENT'), enums.TRADE_MODE.getFieldByKey('SALES_AGENT')].join(
                          ','
                        )
                      }"
                ...
                <base-col v-if="defaultConfig.isSalesPrice" :span="6">
                  <base-form-item label="销售总价" prop="totalSaleMoney">
                    <base-input-number v-model="form.totalSaleMoney" disabled placeholder="请输入销售总价" :precision="2" />
                  </base-form-item>
                </base-col>
                ...
                <base-col v-if="form.basisFlag === enums.WHETHER_STATUS.getFieldByKey('YES')" :span="6">
                  <base-form-item label="货款付款比例" prop="payRate">
                    <base-input-number v-model="form.payRate" disabled placeholder="请输入货款付款比例">
            ...
            <div class="common-operate-block is-special">
              ...
              <base-page
                v-if="showPage"
                ref="orderAgentPurchaseDetailsRef"
                :request="request"
                :columns="columns"
                :tools-config="{
                  setting: {
                ...
                height="380px"
                show-footer
                :name="constant.TABLE_NAME_DOMESTIC_ORDER_DETAILS"
              />
            </div>
