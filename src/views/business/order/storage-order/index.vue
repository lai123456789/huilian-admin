<!-- 仓储订单 -->
<script>
import { getOrderStorage, deleteOrderStorageById } from '@/api/order/order-storage'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getOrderStorage({ ...pager, ...transforArrayPayload(params) })
        return {
          data: data.records,
          total: data.total
        }
  ...
  created() {
    this.helpConfig = {
      show: this.permissions[constant.PERMISSION_BUS_STORAGE],
      mark: constant.PERMISSION_BUS_STORAGE
    }
    this.buttonsConfig = {
      add: {
        visible: this.permissions[constant.PERMISSION_BUS_STORAGE_ADD],
        handler: this.handleAdded
      },
      edit: {
        visible: this.permissions[constant.PERMISSION_BUS_STORAGE_EDIT],
        handler: this.handleEdit
      },
      del: {
        visible: this.permissions[constant.PERMISSION_BUS_STORAGE_DEL],
        handler: this.handleDel
      },
      mores: [
        {
          label: '复制',
          visible: this.permissions[constant.PERMISSION_BUS_STORAGE_COPY],
  ...
  methods: {
    ...
    handleDel(row, cb) {
      this.$confirm('是否确认删除所选记录?')
        .then(() => {
          deleteOrderStorageById(row.id).then(() => {
            // 清空表格全部选中的数据
            this.$refs['storageOrderRef'].handleTableClearCheckbox(true)
    ...
    reLoad(isClearSelect) {
      this.$refs['storageOrderRef'].refresh(isClearSelect)
    }
  }
}
</script>
...
<template>
  <div class="storage-order">
    <!-- 列表页 -->
    <base-page
      ref="storageOrderRef"
      :name="constant.TABLE_NAME_BUS_STORAGE_ORDER"
      :columns="columns"
      :request="request"
