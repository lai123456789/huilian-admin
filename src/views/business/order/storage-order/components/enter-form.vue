<script>
import { getOrderStorageById, saveOrderStorage, updateOrderStorage } from '@/api/order/order-storage'
import { orderStorageDetailImportUrl, orderStorageDetailExportUrl } from '@/api/order/order-storage-detail'
...
export default {
  ...
  data() {
    return {
      enums,
      constant,
      orderStorageDetailImportUrl,
      downloadConfig: {
        url: orderStorageDetailExportUrl,
        params: {},
        fileName: '仓储订单模板'
      },
      // ----------- 固定定义 -----------
  ...
  methods: {
    getFormData() {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          type && getOrderStorageById(this.tableRow.id),
          // 复制订单拿到的id先去请求详情获取数据
          this.copyOrder.enable && getOrderStorageById(this.copyOrder.row.id),
    ...
    handleDataAssemble(info, isDetails, type) {
      const { orderStorageDetailVOList, orderDeliveryTakeVOList, orderDeliverySendVOList } = info
      Object.assign(
        this.form,
        info,
        // 复制过来的订单对下面的字段清空操作
      ...
      if (isDetails) {
        this.$refs.orderDetailsEditTableRef.loadData(
          (orderStorageDetailVOList || []).map(record => ({
            ...record,
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          this.$refs.orderDetailsEditTableRef.validate(valids => {
            ...
            const orderStorageDetailDTOList = this.$refs.orderDetailsEditTableRef.getRecords()
            const formData = Object.assign({ orderStorageDetailDTOList }, this.form)
            if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
              saveOrderStorage(formData)
                .then(() => {
...
<template>
  <base-dialog
    ...
    width="98%"
    custom-class="common-form-dialog storage-order-dialog"
    :title="
    ...
    <base-form ref="formRef" :model="form" :rules="rules" label-width="90px" size="mini">
      ...
      <div class="common-form-block is-special">
        ...
        <base-edit-table
          ...
          height="300px"
          :name="constant.TABLE_NAME_PURCHASE_BUS_STORAGE_ORDER"
          @delete="handleTableDelete"
        >
          <template v-slot:buttons>
            <business-import-button :action="orderStorageDetailImportUrl" :download-config="downloadConfig" @result="handleImportResult" />
          </template>
        </base-edit-table>
      </div>
                        ...
                        <base-col v-if="handleTakeCondition(item.deliveryType)" :span="6">
                          <base-form-item label="提货附件">
                            <base-upload
                              ...
                              :data="{
                                fileModule: constant.FILE_MODULE_STORAGE_ORDER_TAKE_DELIBERY,
                                recordId: form.id,
                                fileType: '普通附件'
                              }"
                          ...
                          <base-col :span="6">
                            <base-form-item label="提货附件">
                              <base-upload
                                ...
                                :on-preview="handleFilePreview"
                                :data="{
                                  fileModule: constant.FILE_MODULE_STORAGE_ORDER_SEND_DELIBERY,
                                  recordId: form.id,
                                  fileType: '普通附件'
                                }"
