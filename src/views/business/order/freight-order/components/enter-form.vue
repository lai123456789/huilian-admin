<script>
import { getOrderAgentFreightById, saveOrderAgentFreight, updateOrderAgentFreight } from '@/api/order/order-agent-freight'
import { orderAgentFreightDetailImportUrl, orderAgentFreightDetailExportUrl } from '@/api/order/order-agent-freight-detail'
...
export default {
  ...
  data() {
    return {
      enums,
      constant,
      orderAgentFreightDetailImportUrl,
      downloadConfig: {
        url: orderAgentFreightDetailExportUrl,
        params: {},
        fileName: '货代订单模板'
      },
      // ----------- 固定定义 -----------
      ...
      rules: {
        ...
        currencyName: [{ required: true, message: '订单币别必填' }],
  ...
  methods: {
    getFormData() {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          type && getOrderAgentFreightById(this.tableRow.id),
          // 复制订单拿到的id先去请求详情获取数据
          this.copyOrder.enable && getOrderAgentFreightById(this.copyOrder.row.id),
    ...
    handleDataAssemble(info, isDetails, type) {
      const { orderAgentFreightDetailVOList, orderDeliveryTakeVOList, orderDeliverySendVOList } = info
      Object.assign(
        this.form,
        info,
        // 复制过来的订单对下面的字段清空操作
      ...
      if (isDetails) {
        this.$refs.orderDetailsEditTableRef.loadData(
          (orderAgentFreightDetailVOList || []).map(record => ({
            ...record,
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          this.$refs.orderDetailsEditTableRef.validate(valids => {
            ...
            const orderAgentFreightDetailDTOList = this.$refs.orderDetailsEditTableRef.getRecords()
            const formData = Object.assign({ orderAgentFreightDetailDTOList }, this.form)
            formData.settleCurrencyName = formData.currencyName
            if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
              saveOrderAgentFreight(formData)
                .then(() => {
              ...
              updateOrderAgentFreight(formData)
                .then(() => {
...
<template>
  <base-dialog
    ...
    width="98%"
    custom-class="common-form-dialog freight-order-dialog"
    :title="
    ...
    <base-form ref="formRef" :model="form" :rules="rules" label-width="90px" size="mini">
        ...
        <base-row :gutter="10">
          ...
          <base-col :span="6">
            <base-form-item label="订单总价" prop="totalMoney">
              <base-input-number v-model="form.totalMoney" readonly placeholder="请输入订单总价" :precision="2" />
            </base-form-item>
          </base-col>
      ...
      <div class="common-form-block is-special">
        ...
        <base-edit-table
          ...
          height="300px"
          :name="constant.TABLE_NAME_PURCHASE_BUS_FREIGHT_ORDER"
          @delete="handleTableDelete"
        >
          <template v-slot:buttons>
            <business-import-button
              :action="orderAgentFreightDetailImportUrl"
                        ...
                        <base-col v-if="handleTakeCondition(item.deliveryType)" :span="6">
                          <base-form-item label="提货附件">
                            <base-upload
                              ...
                              :data="{
                                fileModule: constant.FILE_MODULE_AGENT_FREIGHT_ORDER_TAKE_DELIBERY,
                                recordId: form.id,
                                fileType: '普通附件'
                              }"
                          ...
                          <base-col :span="6">
                            <base-form-item label="提货附件">
                              <base-upload
                                ...
                                :on-preview="handleFilePreview"
                                :data="{
                                  fileModule: constant.FILE_MODULE_AGENT_FREIGHT_ORDER_SEND_DELIBERY,
                                  recordId: form.id,
                                  fileType: '普通附件'
                                }"
