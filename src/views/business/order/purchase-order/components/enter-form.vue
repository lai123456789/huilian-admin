<script>
import { getOrderPurchaseById, saveOrderPurchase, updateOrderPurchase } from '@/api/order/order-purchase'
import { orderPurchaseDetailImportUrl, orderPurchaseDetailExportUrl } from '@/api/order/order-purchase-detail'
...
export default {
  ...
  data() {
    return {
      enums,
      constant,
      orderPurchaseDetailImportUrl,
      downloadConfig: {
        url: orderPurchaseDetailExportUrl,
        params: {},
        fileName: '采购订单模板'
      },
      // ----------- 固定定义 -----------
      ...
      rules: {
        ...
        principalName: [{ required: true, message: '采购主体必填' }],
  ...
  methods: {
    getFormData() {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          type && getOrderPurchaseById(this.tableRow.id),
          // 复制订单拿到的id先去请求详情获取数据
          this.copyOrder.enable && getOrderPurchaseById(this.copyOrder.row.id),
    ...
    handleDataAssemble(info, isDetails, type) {
      const { orderPurchaseDetailVOList, orderDeliveryTakeVOList } = info
      Object.assign(
        this.form,
        info,
        // 复制过来的订单对下面的字段清空操作
      ...
      if (isDetails) {
        this.$refs.orderDetailsEditTableRef.loadData(
          (orderPurchaseDetailVOList || []).map(record => ({
            ...record,
    ...
    handleDetailsColumns() {
      ...
      return [
        ...
        { field: 'lotNo', title: '批号', type: 'input', width: 120 },
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          this.$refs.orderDetailsEditTableRef.validate(valids => {
            ...
            const orderPurchaseDetailDTOList = this.$refs.orderDetailsEditTableRef.getRecords()
            const formData = Object.assign({ orderPurchaseDetailDTOList }, this.form)
            formData.settleCurrencyName = formData.currencyName
            if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
              saveOrderPurchase(formData)
                .then(() => {
...
<template>
  <base-dialog
    ...
    width="98%"
    custom-class="common-form-dialog purchase-order-dialog"
    :title="
    ...
    <base-form ref="formRef" :model="form" :rules="rules" label-width="90px" size="mini">
      ...
      <div class="common-form-block is-special">
        ...
        <base-edit-table
          ...
          height="300px"
          :name="constant.TABLE_NAME_PURCHASE_ORDER_ENTER"
          @delete="handleTableDelete"
        >
          <template v-slot:buttons>
            <business-import-button :action="orderPurchaseDetailImportUrl" :download-config="downloadConfig" @result="handleImportResult" />
          </template>
        </base-edit-table>
      </div>
                        ...
                        <base-col v-if="handleTakeCondition(item.deliveryType)" :span="6">
                          <base-form-item label="提货附件">
                            <base-upload
                              ...
                              :data="{
                                fileModule: constant.FILE_MODULE_PURCHASE_ORDER_TAKE_DELIBERY,
                                recordId: form.id,
                                fileType: '普通附件'
                              }"
