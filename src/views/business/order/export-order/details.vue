...
import {
  getOrderExportById,
  getOrderExportAuditList,
  auditOrderExport,
  getOrderExportStatusById,
  updateByBookCarOrderExport
} from '@/api/order/order-export'
...
import { getOrderExportDetail, updatePriceOrderExportDetail } from '@/api/order/order-export-detail'
...
export default {
  name: 'Details',
  components: { EnterForm, Parts, EnterFormModifyPrice, EnterFormTake, EnterFormSend, EnterFormDetailsTable },
  props: {
    tableRow: {
      type: Object,
      default: () => ({})
  ...
  data() {
    return {
      ...
      requestApi: {
        list: () => getOrderExportAuditList(this.tableRow.id),
        save: auditOrderExport
      },
      showBtns: {
        edit: false,
        del: false,
      ...
      businessExport: {
        visible: false,
        request: async () => {
          const { data } = await getSysReportByMenuMarkSysReportConfig({ menuMark: constant.PERMISSION_BUS_EXPORT_EXPORT })
          return { data }
        }
      },
      financialStatus: [
        ...
        { title: '退税款状态', content: '待退税', processStatus: '0' }
      ],
      statusInfoLength: 0,
      setpsOptions: [
  ...
  mounted() {
    this.showAuditDetailsBtn = this.permissions[constant.PERMISSION_BUS_EXPORT_AUDIT]
    this.dropdownButtonSettleOptions = [
      {
        label: '整单结算',
        visible: this.permissions[constant.PERMISSION_BUS_EXPORT_SETTLE_ZDJS],
        handler: () => this.handleSettleFunc('settle')
      },
      {
        label: '结算货款',
        visible: this.permissions[constant.PERMISSION_BUS_EXPORT_SETTLE_JSHK],
        handler: () => this.handleSettleFunc('settleCargo')
      },
      {
        label: '结算服务费',
        visible: this.permissions[constant.PERMISSION_BUS_EXPORT_SETTLE_JSSDF],
    ...
    this.dropdownButtonJumpOptions = [
      {
        label: '下查归类单',
        visible: this.permissions[constant.PERMISSION_BUS_EXPORT_GL],
        handler: this.jumpClassify
      },
      {
        label: '下查审价单',
        visible: this.permissions[constant.PERMISSION_BUS_EXPORT_SJ],
  ...
  methods: {
    ...
    async getDetailsData(cb) {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          getOrderExportById(this.tableRow.id),
          getOrderExportStatusById(this.tableRow.id),
          ...
          this.showBtns = {
            edit: this.permissions[constant.PERMISSION_BUS_EXPORT_EDIT],
            del: this.permissions[constant.PERMISSION_BUS_EXPORT_DEL],
            export: this.permissions[constant.PERMISSION_BUS_EXPORT_EXPORT]
          }
    ...
    async handleModifyPriceSave(form, done) {
      ...
      await updatePriceOrderExportDetail({ orderDetailId, price, totalMoney })
    ...
    jumpClassificy(rows) {
      jump(this, 'ClassifyBillExport', { openClassifyBillExportDetails: { orderId: rows[0].id } })
    },
    async handleSettleFunc(type) {
      const params = { orderId: this.tableRow.id, orderType: enums.ORDER_TYPE.getFieldByKey('EXPORT') }
      if (type === 'settle') {
    ...
    jumpClassify() {
      jump(this, 'ClassifyBillExport', { orderNo: this.tableRow.orderNo, filtersMark: true })
    },
    jumpAppraisalPrice() {
      jump(this, 'PriceReviewExport', { orderNo: this.tableRow.orderNo, filtersMark: true })
    },
    ...
    detailsReLoad(isClearSelect) {
      this.$refs['exportOrderDetailsRef'].refresh(isClearSelect)
    },
    // 修改某一项时，刷新列表页和详情页
    ...
    handleHiringTaxiesSave() {
      ...
      updateByBookCarOrderExport(this.form)
        .then(() => {
          this.handleUpdateInfoReload(() => {
...
<template>
  <div
    v-loading="loading"
    class="common-operate export-order"
          ...
          <base-form ref="formRef" :model="form" label-width="90px" size="mini">
              ...
              <base-row :gutter="10">
                ...
                <base-col :span="6">
                  <base-form-item label="采购总价" prop="totalPurchaseMoney">
                    <base-input v-model="form.totalPurchaseMoney" disabled />
                  </base-form-item>
                </base-col>
            ...
            <div class="common-operate-block is-special">
              ...
              <base-page
                ...
                height="380px"
                show-footer
                :name="constant.TABLE_NAME_EXPORT_ORDER_DETAILS"
                ...
                <el-collapse-item>
                  ...
                  <div class="content">
                    ...
                    <template v-if="hiringTaxies.type === 'edit'">
                      <el-button type="primary" size="mini" :loading="hiringTaxies.laoding" @click="handleHiringTaxiesSave">保存</el-button>
      ...
      <div class="common-operate-r">
        ...
        <div class="common-operate-r-block">
          ...
          <div v-for="(item, index) in financialStatus" :key="index" class="common-operate-r-item">
            <div class="common-operate-r-label" style="width: 80px">{{ item.title }}:</div>
