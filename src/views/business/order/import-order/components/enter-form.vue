<script>
import { getOrderImportById, saveOrderImport, updateOrderImport } from '@/api/order/order-import'
import { orderImportDetailImportUrl, orderImportDetailExportUrl } from '@/api/order/order-import-detail'
...
export default {
  ...
  data() {
    ...
    return {
      enums,
      constant,
      orderImportDetailImportUrl,
      downloadConfig: {
        url: orderImportDetailExportUrl,
        params: {},
        fileName: '进口订单模板'
      },
  ...
  methods: {
    getFormData() {
      this.$nextTick(() => {
        ...
        Promise.all([
          ...
          type && getOrderImportById(this.tableRow.id),
          // 复制订单拿到的id先去请求详情获取数据
          this.copyOrder.enable && getOrderImportById(this.copyOrder.row.id),
          publicValueSysPublicParamByPublicKey(constant.PARAMS_IMPORT_CUSTOMS_MASTER),
    ...
    handleDataAssemble(info, isDetails, type) {
      const { orderImportDetailVOList, orderDeliveryTakeVOList, orderDeliverySendVOList } = info
      Object.assign(
        this.form,
        info,
        {
      ...
      this.$refs.orderDetailsEditTableRef.loadColumns(this.handleDetailsColumns()).then(() => {
        if (isDetails) {
          this.$refs.orderDetailsEditTableRef.loadData(
            (orderImportDetailVOList || []).map(record => ({
              ...record,
    ...
    handleParts(rows) {
      if (!rows[0].productModel) return this.$message.error('请先选择型号')
      this.parts = {
        visible: true,
        row: rows[0]
      }
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          this.$refs.orderDetailsEditTableRef.validate(valids => {
            ...
            const orderImportDetailDTOList = this.$refs.orderDetailsEditTableRef.getRecords()
            ...
            const isThan = orderImportDetailDTOList.some(item => item.netWeight > item.grossWeight)
            if (isThan) {
...
<template>
  <base-dialog
    ...
    width="98%"
    custom-class="common-form-dialog import-order-dialog"
    :title="
              ...
              <base-row :gutter="10">
                <base-col :span="6">
                  <base-form-item label="境外发货人" prop="overseasShipper">
                    <base-input v-model="form.overseasShipper" placeholder="请输入境外发货人" />
                  </base-form-item>
                </base-col>
        ...
        >
          <template v-slot:buttons="{ selectRecords }">
            <el-button
              ...
              >配件列表</el-button
            >
            <business-import-button :action="orderImportDetailImportUrl" :download-config="downloadConfig" @result="handleImportResult" />
          </template>
        </base-edit-table>
      </div>
                        ...
                        <base-col v-if="handleTakeCondition(item.deliveryType)" :span="6">
                          <base-form-item label="提货附件">
                            <base-upload
                              ...
                              :data="{
                                fileModule: constant.FILE_MODULE_IMPORT_ORDER_TAKE_DELIBERY,
                                recordId: form.id,
                                fileType: '普通附件'
                              }"
                          ...
                          <base-col :span="6">
                            <base-form-item label="提货附件">
                              <base-upload
                                ...
                                :on-preview="handleFilePreview"
                                :data="{
                                  fileModule: constant.FILE_MODULE_IMPORT_ORDER_SEND_DELIBERY,
                                  recordId: form.id,
                                  fileType: '普通附件'
                                }"
