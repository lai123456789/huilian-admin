<!-- 销售订单 -->
<script>
import { getOrderSale, deleteOrderSaleById } from '@/api/order/order-sale'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getOrderSale({ ...pager, ...transforArrayPayload(params) })
        return {
          data: data.records,
          total: data.total
        }
      ...
      columns: [
        ...
        { field: 'principalName', title: '销售主体', link: { name: constant.ROUTE_COMPANY, toField: 'companyName' } },
        { field: 'currencyName', title: '销售币别' },
        {
          field: 'totalProfit',
          title: '销售总利润',
          sum: true,
  ...
  created() {
    this.helpConfig = {
      show: this.permissions[constant.PERMISSION_BUS_SALES],
      mark: constant.PERMISSION_BUS_SALES
    }
    this.buttonsConfig = {
      add: {
        visible: this.permissions[constant.PERMISSION_BUS_SALES_ADD],
        handler: this.handleAdded
      },
      edit: {
        visible: this.permissions[constant.PERMISSION_BUS_SALES_EDIT],
        handler: this.handleEdit
      },
      del: {
        visible: this.permissions[constant.PERMISSION_BUS_SALES_DEL],
        handler: this.handleDel
      },
      mores: [
        {
          label: '复制',
          visible: this.permissions[constant.PERMISSION_BUS_SALES_COPY],
  ...
  methods: {
    ...
    handleDel(row, cb) {
      this.$confirm('是否确认删除所选记录?')
        .then(() => {
          deleteOrderSaleById(row.id).then(() => {
            // 清空表格全部选中的数据
            this.$refs['saleOrderRef'].handleTableClearCheckbox(true)
    ...
    reLoad(isClearSelect) {
      this.$refs['saleOrderRef'].refresh(isClearSelect)
    }
  }
}
</script>
...
<template>
  <div class="sale-order">
    ...
    <base-page
      ref="saleOrderRef"
      :name="constant.TABLE_NAME_SALE_ORDER"
      :columns="columns"
      :request="request"
