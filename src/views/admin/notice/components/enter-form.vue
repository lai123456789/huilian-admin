<script>
import { getSysNoticeById, saveSysNotice, updateSysNotice } from '@/api/admin/sys-notice'
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getSysNoticeById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
