<script>
import { getBaseCustomsRateById, saveBaseCustomsRate, updateBaseCustomsRate } from '@/api/base/base-customs-rate'
  ...
  watch: {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          getDataDictionary([]),
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getBaseCustomsRateById(this.tableRow.id)
        ]).then(([dicts, info]) => {
          this.dicts = dicts
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          ...
          if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
            ...
            updateBaseCustomsRate(formData)
              .then(() => {
      ...
      <base-row>
        ...
        <base-col :span="24">
          <base-form-item label="月份" prop="month">
            <base-date-picker v-model="form.month" type="month" placeholder="选择月份" format="MM" value-format="MM" />
          </base-form-item>
        </base-col>

        <base-col :span="24">
          <base-form-item label="汇率" prop="rate">
            <base-input-number v-model="form.rate" :precision="4" placeholder="请输入汇率">
