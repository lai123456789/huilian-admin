...
export default {
  ...
  created() {
    this.annexConfig = {
      ...
      permissionsAnnex: {
        add: constant.PERMISSION_ACCT_RECEIPT_ANNEX_UPLOAD,
        del: constant.PERMISSION_ACCT_RECEIPT_ANNEX_DEL
      },
      fileModule: constant.FILE_MODULE_ACCT_RECEIPT
    }
  },
  methods: {
    createPage() {
...
<template>
  ...
  >
    <business-details-drawer-tabs
      v-model="tabsActive"
      :options="tabOptions"
      :attrs="{ tableRow, deleteFn: handleDel, reLoad, annexConfig }"
    />
  </business-details-drawer>
</template>
