<script>
import {
  getAcctLetterOfCreditReceiptById,
  saveAcctLetterOfCreditReceipt,
  updateAcctLetterOfCreditReceipt
} from '@/api/finance/acct-letter-of-credit-receipt'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        applicantCompanyType: [{ required: true, message: '申请人类型必填' }],
        applicantCompanyName: [{ required: true, message: '申请人名称必填' }],
        acceptanceCountry: [{ required: true, message: '申请人国家地区必填' }],
      ...
      dicts: {},
      applicantCompanyTypes: []
    }
  },
  computed: {
    applicantCompanyModuleName() {
      let moduleName = ''
      switch (this.form.applicantCompanyType) {
    ...
    value(newValue) {
      ...
      if (newValue) {
        ...
        Promise.all([
          ...
          this.type === enums.FORM_TYPE.getFieldByKey('MODIFY') && getAcctLetterOfCreditReceiptById(this.tableRow.id)
        ]).then(async ([dicts, info]) => {
          this.dicts = dicts
          this.applicantCompanyTypes = this.dicts[DICTS_PAY_COMPANY_TYPE].filter(
      ...
      this.getFormInstance().validate((valid, invalidFields) => {
        if (valid) {
          ...
          if (this.type === enums.FORM_TYPE.getFieldByKey('ADDED')) {
            saveAcctLetterOfCreditReceipt(formData)
              .then(() => {
            ...
            updateAcctLetterOfCreditReceipt(formData)
              .then(() => {
...
<template>
  <base-dialog
    ...
    @save="handleSave"
  >
    <base-form ref="formRef" :model="form" :rules="rules" label-width="120px" size="mini">
      <base-row>
        <base-col :span="6">
        ...
        <base-col :span="6">
          <base-form-item label="收证日期" prop="receiptDate">
            <base-date-picker v-model="form.receiptDate" type="date" placeholder="选择收证日期" />
          </base-form-item>
        </base-col>
      ...
      <base-row>
        <base-col :span="6">
          <base-form-item label="申请人类型" prop="applicantCompanyType">
            <base-select v-model="form.applicantCompanyType" placeholder="请选择申请人类型" :options="applicantCompanyTypes" />
          </base-form-item>
        </base-col>

        <base-col v-if="form.applicantCompanyType" :span="6">
          <base-form-item label="申请人" prop="applicantCompanyName">
            <business-autocomplete-basic
              v-model="form.applicantCompanyName"
              label="申请人"
              placeholder="请输入申请人"
              :module-name="applicantCompanyModuleName"
              :auto-assign="{
                applicantCompanyId: 'companyId'
              }"
              :form="form"
              @clear="() => (form.applicantCompanyId = 0)"
            ></business-autocomplete-basic>
          </base-form-item>
        </base-col>
        <base-col :span="6">
        ...
        <base-col :span="form.applicantCompanyType ? 6 : 12">
      ...
      <base-row>
        <base-col :span="6">
          <base-form-item label="受益人" prop="beneficiaryCompanyName">
            <base-input v-model="form.beneficiaryCompanyName" placeholder="请输入受益人" />
          </base-form-item>
        </base-col>
      ...
      <base-row>
        ...
        <base-col :span="6">
          <base-form-item label="交单期限" prop="deliveryDate">
            <base-date-picker v-model="form.deliveryDate" type="daterange" placeholder="选择交单期限" />
          </base-form-item>
        </base-col>
