<!-- 开证 -->
<script>
import { getAcctLetterOfCreditIssuing, deleteAcctLetterOfCreditIssuingById } from '@/api/finance/acct-letter-of-credit-issuing'
...
export default {
  ...
  data() {
    return {
      ...
      request: async ({ pager, params }) => {
        const { data } = await getAcctLetterOfCreditIssuing({ ...pager, ...transforArrayPayload(params) })
        return {
          data: data.records,
          total: data.total
        }
      ...
      columns: [
        ...
        { field: 'deliveryEndDate', title: '到单期限结束时间' },
        { field: 'deliveryStartDate', title: '到单期限开始时间' },
  ...
  created() {
    this.helpConfig = {
      show: this.permissions[constant.PERMISSION_ACCT_CREDIT_ISSUING],
      mark: constant.PERMISSION_ACCT_CREDIT_ISSUING
    }
    this.buttonsConfig = {
      add: {
        visible: this.permissions[constant.PERMISSION_ACCT_CREDIT_ISSUING_ADD],
        handler: this.handleAdded
      },
      edit: {
        visible: this.permissions[constant.PERMISSION_ACCT_CREDIT_ISSUING_EDIT],
        handler: this.handleEdit
      },
      del: {
        visible: this.permissions[constant.PERMISSION_ACCT_CREDIT_ISSUING_DEL],
  ...
  methods: {
    ...
    handleDel(row, cb) {
      this.$confirm('是否确认删除所选记录?')
        .then(() => {
          deleteAcctLetterOfCreditIssuingById(row.id).then(() => {
            // 清空表格全部选中的数据
            this.$refs['acctCreditIssuingRef'].handleTableClearCheckbox(true)
    ...
    reLoad(isClearSelect) {
      this.$refs['acctCreditIssuingRef'].refresh(isClearSelect)
    }
  }
}
</script>
...
<template>
  <div class="acct-credit-issuing">
    <!-- 列表页 -->
    <base-page
      ref="acctCreditIssuingRef"
      :name="constant.TABLE_NAME_ACCT_CREDIT_ISSUING"
      :columns="columns"
      :request="request"
