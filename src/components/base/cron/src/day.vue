<template lang="html">
  <div :val="value_" class="second-and-minute">
    ...
    <div class="row">
      ...
      <base-input-number v-model="cycle.start" :min="1" :max="31" size="mini" style="width: 100px" @change="type = '2'"></base-input-number>
      <span style="margin-right: 5px; margin-left: 5px">至</span>
      <base-input-number v-model="cycle.end" :min="2" :max="31" size="mini" style="width: 100px" @change="type = '2'"></base-input-number>
      日
    </div>
    <div class="row">
      <el-radio v-model="type" label="3" size="mini" border>循环</el-radio>
      <span style="margin-right: 5px; margin-left: 10px">从</span>
      <base-input-number v-model="loop.start" :min="1" :max="31" size="mini" style="width: 100px" @change="type = '3'"></base-input-number>
      <span style="margin-right: 5px; margin-left: 5px">日开始，每</span>
      <base-input-number v-model="loop.end" :min="1" :max="31" size="mini" style="width: 100px" @change="type = '3'"></base-input-number>
      日执行一次
    </div>
    <div class="row">
      <el-radio v-model="type" label="8" size="mini" border>工作日</el-radio>
      <span style="margin-right: 5px; margin-left: 10px">本月</span>
      <base-input-number v-model="work" :min="1" :max="7" size="mini" style="width: 100px" @change="type = '8'"></base-input-number>
      号，最近的工作日
    </div>
    <div class="row">
    ...
    <div class="row">
      ...
      <el-checkbox-group v-model="appoint">
        <div v-for="i in 4" :key="i" style="line-height: 18px">
          <template v-for="j in 10">
            <el-checkbox
              v-if="parseInt(i - 1 + '' + (j - 1)) < 32 && !(i === 1 && j === 1)"
              :key="j"
              :label="i > 1 ? i - 1 + '' + (j - 1) : '' + (j - 1)"
              @change="type = '4'"
              >{{ i - 1 + '' + (j - 1) }}</el-checkbox
            >
          </template>
