...
export default {
  name: 'BusinessAutocompleteFormula',
  components: { Grid },
  inject: {
    elForm: {
      default: ''
    ...
    config() {
      return {
        fetchSuggestions: (payload, callback) => {
          getBaseSettlementFormula(payload).then(res => {
...
<template>
  <div class="business-autocomplete-formula">
    <base-autocomplete
    ...
    <base-dialog class="business-autocomplete-formula-dialog" width="1100px" :title="label" :visible.sync="visible" :show-footer="false">
