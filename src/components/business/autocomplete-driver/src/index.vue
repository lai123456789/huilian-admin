...
export default {
  name: 'BusinessAutocompleteDriver',
  components: { Grid },
  inject: {
    elForm: {
      default: ''
    ...
    config() {
      return {
        fetchSuggestions: (payload, callback) => {
          getCompanyTranDriver(payload).then(res => {
...
<template>
  <div class="business-autocomplete-driver">
    <base-autocomplete
    ...
    <base-dialog class="business-autocomplete-driver-dialog" width="1100px" :title="label" :visible.sync="visible" :show-footer="false">
      <grid
        v-if="visible"
